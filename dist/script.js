...
class JogoVelha {
  ...
  initElements () {
    ...
    this.saveServer = document.querySelector('#send')
    this.saveServer.addEventListener('click', this.send.bind(this))
  ...
  toRender () {
    ...
    if (winner == 'X' || winner == 'O') {
      this.end = true
      this.saveServer.style.display = "block";
      this.modal(`O jogador ${winner} venceu!`)
    } else {
        this.saveServer.style.display = 'none'
    }
  ...
  checkWinner () {
    ...
    const valueX = parseInt(
      this.played.map(value => (value == 'X' ? 1 : 0)).join(''),
      2
    )
    // retrieve decimal sequence play-0
    const valueO = parseInt(
      this.played.map(value => (value == 'O' ? 1 : 0)).join(''),
      2
    )
  ...
  send () {
    // get user name
    const playx = this.playx.value
    const playo = this.play0.value
    ...
    domtoimage.toPng(this.velha, {width: '400', height: '400'})
      .then((dataUrl) => {
        ...
        return axios.post('/send', {
            playx, playo, played: JSON.stringify(this.played),
            img: dataUrl
        })        
      })
      .then((response) => {
